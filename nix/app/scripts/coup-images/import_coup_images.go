...
// ImportCoupRoleImages copies supported role image files into canonical ID-based filenames.
func ImportCoupRoleImages(sourceDir, outputDir string) (ImportResult, error) {
	...
	for _, roleImage := range game.CoupRoleImageManifest() {
		...
		if err := removeExistingCanonicalImages(outputDir, roleImage.ID); err != nil {
			return result, err
		}

		outputPath := filepath.Join(outputDir, fmt.Sprintf("%d%s", roleImage.ID, extension))
...
func removeExistingCanonicalImages(outputDir string, roleID int) error {
	for _, extension := range game.SupportedCoupRoleImageExtensions() {
		path := filepath.Join(outputDir, fmt.Sprintf("%d%s", roleID, extension))
